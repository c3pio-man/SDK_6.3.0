...
public:
    ...
    ~CfgWrapper();
    template<typename T> T read(const std::string& name, T default_value = T());
...
template<> inline int CfgWrapper::read<int>(const std::string& name, int default_value) {
    return ReadInt(cfg_, name.c_str(), default_value);
}
template<> inline std::string CfgWrapper::read<std::string>(const std::string& name, std::string default_value) {
    return ReadString(cfg_, name.c_str(), default_value.c_str());
}

template<> inline CfgSecret CfgWrapper::read<CfgSecret>(const std::string& name, CfgSecret default_value) {
