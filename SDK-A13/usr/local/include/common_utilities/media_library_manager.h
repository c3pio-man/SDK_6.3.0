...
public:
    ...
    std::vector<Track> getTracks(SortType sort, TrackGroupType filter_type, std::string filter_value, bool filter_strict = true);
    std::vector<TrackGroup> getTrackGroups(TrackGroupType group_type, SortDir dir);
    std::vector<TrackGroup> getSubdirsTracksCount(std::string dir_path);
